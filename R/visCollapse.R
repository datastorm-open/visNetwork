...
#' 
visCollapse <- function(graph, nodes, fit = FALSE, resetHighlight = TRUE, clusterOptions = NULL){
  ...
  data <- list(id = graph$id, nodes = nodes, fit = fit, resetHighlight = resetHighlight)
...
#'
#' @export  
visUncollapse <- function(graph, nodes = NULL, fit = FALSE, resetHighlight = TRUE){
  
  if(!any(class(graph) %in% "visNetwork_Proxy")){
    stop("Can't use visUncollapse with visNetwork object. Only within shiny & using visNetworkProxy")
  }
  
  data <- list(id = graph$id, fit = fit, resetHighlight = resetHighlight)
